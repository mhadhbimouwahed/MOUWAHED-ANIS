...
getSession(id: number) {return SESSIONITEMS[id -1];}
countSession(){return SESSIONITEMS.length;}
sessionpartrack(track: string): number{
    let ms=0;
    for (let sessionn of SESSIONITEMS){
      if(sessionn.track == track) ms++;
    }
    return ms;  
  }
}